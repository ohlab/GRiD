...
while getopts ":h" opt; do
  case ${opt} in
    h )
      echo "Usage:"
      echo "    grid.sh -h                     Display this help message."
      echo "    grid.sh single <options>       GRiD using a single genome"
      echo "    grid.sh multiplex <options>    GRiD high throughput"
      exit 0
      ;;
   \? )
...
case "$subcommand" in
  ...
  single)
   ...
   LIST=false
    ...
    while getopts ":r:o:g:l:h" opt; do
      case ${opt} in
        r )
          READS_DIR=$OPTARG
          ;;
...
###############
 multiplex)
   ...
   MERGE=false
    # Process package options
    while getopts ":r:o:d:c:t:l:pmh" opt; do
      case ${opt} in
        r )
          READS_DIR=$OPTARG
          ;;
        ...
        h )
      echo "Usage:"
      echo "    grid.sh multiplex <options>"
      echo "    <options>"
      echo "    -r         Reads directory (single end reads)"
      echo "    -o         Output directory"
      echo "    -d         GRiD database directory"
      echo "    -c  FLOAT  Coverage cutoff (>= 0.2) [default 1]"
      echo "    -p         Enable reassignment of ambiguous reads"
...
if [ $THETA -gt 0 ] && [ "$PATHO" == "false" ]; then
    ...
    echo "Use grid.sh multiplex -h for usage information "
    exit 1
fi
if [ "$LIST" != "false" ]; then
LIS=$(readlink -f $LIST)
...
bowtie2-build $GENOME bowtie_$ref_genome
usearch -ublast $GENOME -db $GRID_DIR/dnaa.udb -strand both -evalue 0.05 -blast6out dnaa.txt
usearch -ublast $GENOME -db $GRID_DIR/dif.udb -strand both -evalue 0.05 -blast6out dif.txt

dnaa_fragment=$(sort -nr -k12,12 dnaa.txt | head -1 | cut -f1)
dnaa_start=$(sort -nr -k12,12 dnaa.txt | head -1 | cut -f7)

dif_fragment=$(sort -nr -k12,12 dif.txt | head -1 | cut -f1)
dif_start=$(sort -nr -k12,12 dif.txt | head -1 | cut -f7)
...
bowtie2 -x bowtie_$ref_genome -U $i.MAP.subsample.fastq --very-sensitive -S $i.sub1.sam

seqtk sample -s90 $i.MAP.fastq 0.85 > $i.MAP.subsample2.fastq
bowtie2 -x bowtie_$ref_genome -U $i.MAP.subsample2.fastq --very-sensitive -S $i.sub2.sam
...
do
bowtie2 -x $DBR/$BOWTIE -U $i.fastq --very-sensitive -S $ODR/$i.$BOWTIE.main.sam
done
...
for f in $i.*.main.sam
do
samtools view -H $f >> $i.headers.allSeq.sam
done

for f in $i.*.main.sam
do
samtools view $f >> $i.allSeq.no.headers.sam
done
...
if [ "$PATHO" == "true" ]; then
pathoscope2.py ID -alignFile $i.allSeq.sam -fileType sam -outDir . -thetaPrior $THETA
rm $i.allSeq.sam
rm *report.tsv
