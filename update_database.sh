...
grep -w -f check_GRiD_genomes.temp.txt $GRID_DIR/database_misc.txt | cut -f1 > check_GRiD_genomes.txt
...
awk -F'\t' '$2=="" {$2="1,10000000p"};1' OFS='\t' $DBR/GRiD.temp.misc.txt  >> $GRID_DIR/database_misc.txt
rm $DBR/GRiD.temp.misc.txt
rm grid_database_*

cd $DBR
ls BOWTIE_$NAME.rev.1.bt2* | rev | cut -d'.' -f4- | rev >> $GRID_DIR/bowtie.txt 
