...
do
bowtie2 -x $ODR/bowtie_$ref_genome -U $i.fastq --very-sensitive -S $ODR/$i.main.sam --al $ODR/$i.MAP.fastq -p $NUM_THREAD
...
do
bowtie2 -x $DBR/$BOWTIE -U $i.fastq --very-sensitive -S $ODR/$i.$BOWTIE.main.sam -p $NUM_THREAD
done
...
#############################################################
######## error check ##########
errorCheck=$(samtools view -c -b -F 4 $i.sorted.bam )
echo "checking for errors in BAM file"
...
grep -w -f $i.passed_genomes.tmp.txt $GRID_DIR/database_misc.txt | awk '$5 < 90' | cut -f1 > $i.passed_genomes.txt
else
grep -w -f $i.passed_genomes.tmp.txt $GRID_DIR/database_misc.txt | cut -f1 > $i.passed_genomes.txt
fi
...
grep -w "$GENOME" $GRID_DIR/database_misc.txt | cut -f2 | sed 's/p,/p\n/g' > $i.$GENOME.chunk.txt
grep -w "$GENOME" $GRID_DIR/database_misc.txt | cut -f3 > $i.$GENOME.remove.txt
...
do
len=$(grep -w "$f" $GRID_DIR/database_misc.txt | cut -f4)
if [ $len -lt 4000000 ] 
then
...
######## error check ##########
echo "checking for errors in GRiD output"
errorCheck=$(wc -l $i.GRiD.txt | cut -f1 -d' ')
if [ $errorCheck == 0 ]; then
echo "Error in GRiD output"
rm $i.*.GRiD.txt
rm $i.sorted.bam.bai
rm $i.filtered.bam
